...
<section style="padding: 120px 5% 60px; min-height: 70vh;">
   <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
      <div>
         <h1
            style="margin: 0 0 4px 0; font-family: 'Cormorant Garamond', serif; font-size: clamp(2rem, 5vw, 3rem); font-weight: 400;">Settings - Permainan</h1>
         <p style="margin: 0;">Klik pada baris tabel untuk meng-edit data wahana Permainan.</p>
   ...
   <div style="margin-bottom: 40px;">
      ...
      <div style="display:flex; justify-content:center; align-items:center; margin-top:12px; gap:8px;">
         ...
         {% else %}
            <a href="{{ url('settings/permainan') }}?{{ http_build_query({'search': search, 'page': prevPage, 'per_page': perPage}) }}"
         ...
         {% for i in 1..totalPages %}
            ...
            {% else %}
               <a href="{{ url('settings/permainan') }}?{{ http_build_query({'search': search, 'page': i, 'per_page': perPage}) }}"
         ...
         {% else %}
            <a href="{{ url('settings/permainan') }}?{{ http_build_query({'search': search, 'page': nextPage, 'per_page': perPage}) }}"
...
<div id="editModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(600px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      ...
      <form method="post" action="{{ url('settings/update_wahana') }}" enctype="multipart/form-data">
...
<div id="addModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(600px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      ...
      <form method="post" action="{{ url('settings/create_wahana') }}" enctype="multipart/form-data">
         <input type="hidden" name="MAX_FILE_SIZE" value="5242880">
