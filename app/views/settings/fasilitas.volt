...
<section style="padding: 120px 5% 60px; min-height: 70vh;">
   <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
      <div>
         <h1
            style="margin: 0 0 4px 0; font-family: 'Cormorant Garamond', serif; font-size: clamp(2rem, 5vw, 3rem); font-weight: 400;">Settings - Fasilitas</h1>
         <p style="margin: 0;">Klik pada baris tabel untuk meng-edit data Fasilitas.</p>
         <form id="searchForm" method="GET"
            style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-top:12px;">
            <input type="text" name="search" placeholder="Cari Fasilitas..." value="{{ search }}"
            ...
            <a href="{{ url('settings/fasilitas') }}" class="btn-reset">Reset</a>
            {% endif %}
         </form>
      </div>
      <div>
   ...
   <div class="table-responsive" style="overflow-x: auto; background: rgba(16, 36, 23, 0.6); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; padding: 10px;">
      <div style="margin-bottom:8px; color:#e8cc7a; font-size:0.9rem;">
         Menampilkan {{ ((currentPage - 1) * perPage) + 1 }} – {{ ((currentPage - 1) * perPage) + fasilitasList|length }} dari
         {{ totalData }} data
      </div>
      <table class="table table-sm text-white" style="width:100%; border-collapse: collapse;">
         <thead>
            <tr style="background: rgba(0,0,0,0.2);">
               <th style="text-align:left; padding:12px 10px; border-bottom:2px solid rgba(255,255,255,.15);">Nama Fasilitas</th>
   ...
   <div style="display:flex; justify-content:center; align-items:center; margin-top:12px; gap:8px;">
      ...
      {% else %}
         <a href="{{ url('settings/fasilitas') }}?{{ http_build_query({'search': search, 'page': prevPage, 'per_page': perPage}) }}"
      ...
      {% for i in 1..totalPages %}
         ...
         {% else %}
            <a href="{{ url('settings/fasilitas') }}?{{ http_build_query({'search': search, 'page': i, 'per_page': perPage}) }}"
      ...
      {% else %}
         <a href="{{ url('settings/fasilitas') }}?{{ http_build_query({'search': search, 'page': nextPage, 'per_page': perPage}) }}"
...
<div id="editModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(640px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      <h3 style="margin:0 0 18px; font-size: 1.5rem; color:#d4b15a; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px;">Edit Fasilitas</h3>
      <form method="post" action="{{ url('settings/update_fasilitas') }}">
         <input type="hidden" name="id" id="editId">
         <input type="hidden" name="img_url" id="editImgUrl" value="">

         <div style="margin-bottom:14px;">
            <label for="editNama" style="display:block; margin-bottom:6px; font-weight: 600;">Nama Fasilitas</label>
...
<div id="addModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(640px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      <h3 style="margin:0 0 18px; font-size: 1.5rem; color:#d4b15a; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px;">Tambah Data Fasilitas</h3>
      <form method="post" action="{{ url('settings/create_fasilitas') }}">
         <input type="hidden" name="img_url" id="addImgUrl" value="">

         <div style="margin-bottom:14px;">
            <label for="addNama" style="display:block; margin-bottom:6px; font-weight: 600;">Nama Fasilitas</label>
...
(function () {
   ...
   async function chunkUploadFile(file, progressBar, progressText, progressContainer, resourceUrlInput) {
      ...
      try {
         const metaResp = await fetch("{{ url('settings/save_extension_fasilitas') }}", {
            method: 'POST',
            body: metaForm,
         });
         ...
         for (let i = 0; i < totalChunks; i++) {
            ...
            const resp = await fetch("{{ url('settings/chunk_upload_fasilitas') }}", {
               method: 'POST',
               body: chunkForm,
            });
   ...
   addFormEl.addEventListener('submit', async function (e) {
      ...
      if (!file) {
         alert('Pilih gambar fasilitas terlebih dahulu.');
         return;
      }
