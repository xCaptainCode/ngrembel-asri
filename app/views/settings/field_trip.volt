...
<section style="padding: 120px 5% 60px; min-height: 70vh;">
   <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 12px;">
      <div>
         <h1
            style="margin: 0 0 4px 0; font-family: 'Cormorant Garamond', serif; font-size: clamp(2rem, 5vw, 3rem); font-weight: 400;">Settings - Field Trip</h1>
         <p style="margin: 0;">Klik pada baris tabel untuk meng-edit data wahana Field Trip.</p>
         <form id="searchForm" method="GET"
            style="display:flex; gap:8px; align-items:center; flex-wrap:wrap; margin-top:12px;">
            <input type="text" name="search" placeholder="Cari field trip..." value="{{ search }}"
            ...
            <a href="{{ url('settings/field_trip') }}" class="btn-reset">Reset</a>
            {% endif %}
         </form>
      </div>
      <div>
   ...
   <div style="margin-bottom: 40px;">
      ...
      <p>Data Wahana Field Trip belum tersedia.</p>
      {% else %}
      ...
      <div style="display:flex; justify-content:center; align-items:center; margin-top:12px; gap:8px;">
         ...
         {% else %}
            <a href="{{ url('settings/field_trip') }}?{{ http_build_query({'search': search, 'page': prevPage, 'per_page': perPage}) }}"
         ...
         {% for i in 1..totalPages %}
            ...
            {% else %}
               <a href="{{ url('settings/field_trip') }}?{{ http_build_query({'search': search, 'page': i, 'per_page': perPage}) }}"
         ...
         {% else %}
            <a href="{{ url('settings/field_trip') }}?{{ http_build_query({'search': search, 'page': nextPage, 'per_page': perPage}) }}"
...
<div id="editModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(600px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      <h3 style="margin:0 0 18px; font-size: 1.5rem; color:#d4b15a; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px;">Edit Wahana Field Trip</h3>
      
      <form method="post" action="{{ url('settings/update_field_trip') }}" enctype="multipart/form-data">
...
<div id="addModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.65); z-index:9999; align-items:center; justify-content:center; backdrop-filter: blur(4px);">
   <div style="width:min(600px, 92vw); background:#102417; color:#fff; border:1px solid rgba(255,255,255,.2); border-radius:16px; padding:24px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
      ...
      <form method="post" action="{{ url('settings/create_field_trip') }}" enctype="multipart/form-data">
         <input type="hidden" name="MAX_FILE_SIZE" value="5242880">
