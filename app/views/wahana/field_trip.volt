...
<section id="wahana" style="padding: 2rem 5% 8rem;">
   {% if wahanaList is empty %}
      <p style="text-align: center; color: rgba(255,255,255,0.6); margin-top: 2rem;">Tidak ada paket field trip aktif saat ini.</p>
   {% else %}
      <div class="masonry-grid" id="wahanaGrid">
         {% for item in wahanaList %}
            ...
            {% if not image_src %}
               ...
               {% else %}
                  {% set image_src = 'https://images.unsplash.com/photo-1544025162-d76694265947?w=1400&q=80' %}
               {% endif %}
            {% endif %}
